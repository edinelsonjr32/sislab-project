...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('equipamento.store') }}" autocomplete="off" class="form-horizontal" enctype="multipart/form-data">
            @csrf
            @method('post')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Adicionar Equipamento') }}</h4>
                <p class="card-category"></p>
              </div>
              <div class="card-body ">
                <div class="row">
                  <div class="col-md-12 text-right">
                      <a href="{{ route('equipamento.index') }}" class="btn btn-sm btn-primary">{{ __('Voltar para Listagem de Equipamento') }}</a>
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('tombo') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('tombo') ? ' is-invalid' : '' }}" name="tombo" id="input-tombo" type="text" placeholder="{{ __('tombo') }}" value="{{ old('tombo') }}" required="true" aria-required="true"/>
                      @if ($errors->has('tombo'))
                  ...
                  <div class="col-sm-7">
                    <div class="form-group bmd-form-group">
                        <div class="form-group{{ $errors->has('descricao') ? ' has-danger' : '' }}">
                                <textarea cols="30" rows="10" class="form-control{{ $errors->has('descricao') ? ' is-invalid' : '' }}" name="descricao" id="input-descricao" type="text" placeholder="Descrição" required="true" aria-required="true"></textarea>
